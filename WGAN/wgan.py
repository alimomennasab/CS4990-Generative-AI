...
class Critic(nn.Module):
    ...
    def forward(self, x, genre):
        ...
        return logits # wgan uses raw score instead of sigmoid
...
def train_wgan(generator, critic, dataloader, g_optimizer, d_optimizer, device='cpu',
               num_epochs=20, run_folder=None, clip_value=0.01, critic_iters=5):
    generator.to(device)
    critic.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (input_seq, target_seq, genre) in enumerate(progress_bar):
            ...
            for _ in range(critic_iters):
                ...
                # Real
                real_validity = critic(target_seq, genre)
                ...
                fake_validity = critic(fake_seq, genre)
    ...
    plt.plot(d_losses, label="Critic (D) Loss")
    plt.plot(g_losses, label="Generator Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("WGAN Training Losses (with weight clipping)")
    plt.legend()
    plt.grid(True)
...
def main():
    ...
    critic = Critic(vocab_size, 128, 256, 16)

    g_optimizer = optim.Adam(generator.parameters(), lr=0.0002)
    d_optimizer = optim.Adam(critic.parameters(), lr=0.0002)
    criterion = nn.BCELoss()

    train_wgan(generator, critic, dataloader, g_optimizer, d_optimizer, 
           device=device, num_epochs=epochs, run_folder=run_folder)
